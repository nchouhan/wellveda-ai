...
export default function FAQDrawer({ language, onSelectQuestion }: FAQDrawerProps) {
  const [expanded, setExpanded] = useState(false);
  const uiLanguage = language === "hi" ? "hindi" : "english";
  ...
  const visibleQuestions = expanded 
    ? faqQuestions[uiLanguage] 
    : faqQuestions[uiLanguage].slice(0, 10);
    ...
    <Sheet>
      <SheetTrigger asChild>
        <Button 
          variant="outline" 
          size="sm" 
          className="fixed right-4 top-20 z-30 flex items-center gap-1 shadow-md"
        >
          <HelpCircle className="h-4 w-4" />
          <span className={uiLanguage === "hindi" ? "font-hindi" : ""}>
            {uiLanguage === "english" ? "Common Questions" : "सामान्य प्रश्न"}
          </span>
        </Button>
      </SheetTrigger>
      <SheetContent className="w-[350px] sm:w-[450px] overflow-y-auto">
        <SheetHeader>
          <SheetTitle className={uiLanguage === "hindi" ? "font-hindi" : ""}>
            {uiLanguage === "english" ? "Commonly Asked Questions" : "अक्सर पूछे जाने वाले प्रश्न"}
          </SheetTitle>
        </SheetHeader>
        ...
        <div className="mt-6">
          <Accordion type="single" collapsible className="w-full">
            {visibleQuestions.map((question, index) => (
              <AccordionItem key={index} value={`item-${index}`}>
                <AccordionTrigger className={`text-sm text-left ${uiLanguage === "hindi" ? "font-hindi" : ""}`}>
                  {question}
                </AccordionTrigger>
                <AccordionContent>
                  <Button 
                    ...
                    className={`p-0 h-auto text-primary ${uiLanguage === "hindi" ? "font-hindi" : ""}`}
                  >
                    {uiLanguage === "english" ? "Ask this question" : "यह प्रश्न पूछें"}
                  </Button>
                </AccordionContent>
              </AccordionItem>
            ))}
          ...
          <div className="text-center mt-4">
            <Button 
              ...
              className={`text-sm ${uiLanguage === "hindi" ? "font-hindi" : ""}`}
            >
              {expanded
                ? (uiLanguage === "english" ? "Show Less" : "कम दिखाएं")
                : (uiLanguage === "english" ? "Show More Questions" : "अधिक प्रश्न दिखाएं")}
            </Button>
          </div>
        </div>
      </SheetContent>
